...
TEST_F(StatsIsolatedStoreImplTest, All) {
  ...
  EXPECT_EQ("c1", c1.tagExtractedName(store_.symbolTable()));
  EXPECT_EQ("scope1.c2", c2.tagExtractedName(store_.symbolTable()));
  EXPECT_EQ(0, c1.tags(store_.symbolTable()).size());
  EXPECT_EQ(0, c1.tags(store_.symbolTable()).size());
  ...
  EXPECT_EQ("g1", g1.tagExtractedName(store_.symbolTable()));
  EXPECT_EQ("scope1.g2", g2.tagExtractedName(store_.symbolTable()));
  EXPECT_EQ(0, g1.tags(store_.symbolTable()).size());
  EXPECT_EQ(0, g1.tags(store_.symbolTable()).size());
  ...
  EXPECT_EQ("h1", h1.tagExtractedName(store_.symbolTable()));
  EXPECT_EQ("scope1.h2", h2.tagExtractedName(store_.symbolTable()));
  EXPECT_EQ(0, h1.tags(store_.symbolTable()).size());
  EXPECT_EQ(0, h2.tags(store_.symbolTable()).size());
  h1.recordValue(200);
  h2.recordValue(200);
